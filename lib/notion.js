...
function getNotionClient() {
  if (!notionClient) {
    notionClient = new Client({ auth: process.env.NOTION_TOKEN, timeoutMs: 120000 });
  }
  return notionClient;
}
...
async function withRetry(fn) {
  ...
  while (attempt < maxAttempts) {
    try {
      ...
      if (!isRetryable && !isNetwork) break;
...
export async function getChildren(pageId, notion = null) {
  if (!notion) {
...
export async function fetchHierarchy(rootPageId) {
  // Get Notion client once for the entire hierarchy
  const notion = getNotionClient();
  
  let siteTitle = "Documentation";
  try {
    const root = await withRetry(() => notion.pages.retrieve({ page_id: rootPageId }));
    ...
    console.error("Error fetching root page:", e.message);
  }

  const rootChildren = await getChildren(rootPageId, notion);
